#!/usr/bin/env python3
"""Rank vanilla-LSTM runs in results/experiments.jsonl for MODEL SELECTION.
...
The sibling ``analyze_experiments.py`` assumes the CNN-LSTM schema (it reads
``inception_filters``) and ranks on test, so it both KeyErrors on LSTM runs and
selects the wrong way. This version selects only vanilla-LSTM records and shows
the levers that matter for them: lr, weight decay, hidden, num_layers, dropout,
train_stride. ``ftrain`` (final train_loss) is surfaced to spot under-training
(value near 1.0 == still predicting the mean).

Requires runs produced after the val-metrics patch to lstm.py / lstm_mlp.py
(records with ``metrics.validation``). Older runs lack it and are listed
separately at the bottom -- re-run them to rank on validation.

Usage:
    python scripts/analyze_lstm_baseline_experiments.py                 # all LSTM runs
    python scripts/analyze_lstm_baseline_experiments.py --prefix ls1-   # one sweep stage
    python scripts/analyze_lstm_baseline_experiments.py --top 5
    python scripts/analyze_lstm_baseline_experiments.py --prefix ls3- --group
...
def is_vanilla_lstm(rec) -> bool:
    """Vanilla-LSTM records only: model string starts 'LSTM (' -- the trailing
    space excludes 'LSTM-MLP (...)' and 'CNN-LSTM (...)'."""
    return str(rec.get("model", "")).startswith("LSTM (")
...
def composite(m) -> float:
    ...
    Same blend as analyze_experiments.py: directional edge over 0.5 (x2), plus
...
_CONFIG_KEYS = ("lr", "wd", "hidden", "nl", "drop", "tr_stride")

_HDR = (f"{'rank':>4} {'tag':<22} {'lr':>6} {'wd':>6} {'hid':>4} {'nl':>3} "
        f"{'drop':>5} {'strd':>5} {'sd':>3} {'ftrain':>8} "
...
def print_runs(rows):
    ...
    for i, d in enumerate(ranked, 1):
        print(f"{i:>4} {d['tag']:<22} {d['lr']:>6.0e} {d['wd']:>6.0e} "
              f"{d['hidden']:>4} {d['nl']:>3} {d['drop']:>5.2f} "
              f"{d['tr_stride']:>5} {d['seed']:>3} {d['final_train']:>8.4f} "
    ...
    if unranked:
        ...
        for d in unranked:
            print(f"      {d['tag']:<22} (lr={d['lr']:g} strd={d['tr_stride']} "
...
def print_grouped(rows):
    ...
    for i, a in enumerate(agg, 1):
        ...
        print(f"{i:>4} {k['lr']:>6.0e} {k['wd']:>6.0e} {k['hidden']:>4} "
              f"{k['nl']:>3} {k['drop']:>5.2f} {k['tr_stride']:>5} {a['n']:>2} "
...
def main():
    ...
    if not recs:
        raise SystemExit("No matching vanilla-LSTM runs in results/experiments.jsonl.")
    ...
    if args.group:
        ...
        print(f"\nBEST config by VALIDATION (seed-averaged): lr={best['lr']:g} "
              ...
              f"dropout={best['drop']:g} train_stride={best['tr_stride']}")
